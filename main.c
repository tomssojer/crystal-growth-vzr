    ...
    for(int i=0;i<vrstice;i++) {
        for (int j = COLUMNS-i-1; j < stolpci-i;j+=2) {
            ...
            if(i-1 >=0) { // zgornje sosede
                if(j-1>=0 && j!= COLUMNS-i-1) { //zgornje LEVA
                    sosede[0][0] = j-1;
                    sosede[0][1] = i-1;
                ...
                } else { // zgoraj ni desne sosede
                    sosede[1][0] = -1;
                    sosede[1][1] = -1;
                }
            } else  { // zgornjih sosed NI
                sosede[0][0] = -1;
                sosede[0][1] = -1;
                sosede[1][0] = -1;
                sosede[1][1] = -1;
            ...
            if(j-2>=0 && j!= COLUMNS-i-1) { // ISTA VRSTICA; LEVA
                sosede[2][0] = j-2;;
                sosede[2][1] = i;
            ...
            if(j+2<stolpci && j!= stolpci-i) { // DESNA soseda
                sosede[3][0] = j+2;
                sosede[3][1] = i;
                mapped_sosede[3]=i*COLUMNS + mappJ +2;
            } else  { //  ni desne sosede
                sosede[3][0] = -1;
                sosede[3][1] = -1;
            }

            if(i+1 <vrstice && j!= COLUMNS-i-1) { // spodnje  sosede  //i+1 || j-1 in j+1
                if(j-1>=0) { //leva
                    sosede[4][0] = j-1;
                    sosede[4][1] = i+1;
                ...
                if(j+1<stolpci && j!= stolpci-i) { // desna spodnja soseda
                    sosede[5][0] = j+1;
                    sosede[5][1] = i+1;
